...
- getrange：获取指定区间范围内的值，类似 between...and 的关系；从零到负一表示全部
  - 格式是：`getrange <key> <起始位置> <结束位置>`
- setrange：用 `<value>` 覆写 `<key>` 所储存的字符串值
  - 格式是 `setrange <key> <起始位置> <value>`
...
- setex(set with expire)：设置键值的同时，设置过期时间，单位秒
  - `setex <key> <过期时间> <value>`
- setnx(set if not exist)：只有在 key 不存在时，设置 key 的值
  - 格式是：`setnx <key> <value>`
...
- lrem 删除 n 个指定值：`lrem listname 删N个 value值`
- ltrim 截取指定范围的值后再赋值给 key：`ltrim listname 开始index 结束index`
...
- rpoplpush：从 `<key1>` 列表右边吐出一个值，插到 `<key2>` 列表左边
- 格式是：`rpoplpush 源列表 目的列表`
...
- lset 将指定列表下标为 index 的值替换成 value；格式是：`lset listname index value`
- linsert 插入值：`linsert listname before/after 指定值 newvalue`
...
Redis 的 Set 是 string 类型的无序集合，它底层其实是一个 value 为 null 的 hash 表
...
- sadd：添加 set 集合
- smembers：查看 set 集合
- sismember：查看指定值是否是该集合中的成员 `sismember setname 指定key`
...
- `spop key`：随机出栈
- `smove setname1 setname2`：将 setname1 里的某个值赋给 setname2
...
- hlen：hash 的 key 个数
- hexists：`hexists key` 在 key 里面的某个值的 key
- hkeys：hash 的所有 key
- hvals：hash 的所有 values
...
- hincrby：哈希表 key 中的域 field 的值加上增量 1
- hincrbyfloat：哈希表 key 中的域 field 的值加上减量 1
...
- zcard：统计整个集合个数
- zcount：统计该集合，分数区间内的元素个数 `zcount key score区间`
- zrank：返回该值在集合中的排名，从 0 开始 `zrank key 对应值`
- zscore：获得分数值 `zscore key 对应值`
...
- 格式：`setbit <key> <offset> <value>` 设置 Bitmaps 中某个偏移量的值（0 或 1）
- 格式：`getbit <key> <offset>` 获取 Bitmaps 中某个偏移量的值
...
格式：`bitcount <key> [start end]` 统计字符串从 start 字节到 end 字节比特值为 1 的数量
...
:::
::: tab pfcount
格式：`pfcount <key> [key ...]` 计算 HLL 的近似基数，可以计算多个 HLL，比如用 HLL 存储每天的 UV，计算一周的 UV 可以使用 7 天的 UV 合并计算即可
...
:::
::: tab pfmerge
格式：`pfmerge <destkey> <sourcekey> [sourcekey ...]` 将一个或多个 HLL 合并后的结果存储在另一个 HLL 中，比如每月活跃用户可以使用每天的活跃用户来合并计算可得
